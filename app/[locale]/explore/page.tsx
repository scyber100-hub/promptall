...
const AI_TOOL_LABELS: Record<string, string> = {
  ...
  dalle: 'DALL-E', 'stable-diffusion': 'Stable Diffusion', copilot: 'Copilot', perplexity: 'Perplexity', other: 'Other',
};
...
const CATEGORY_LABELS: Record<string, string> = {
  writing: 'Writing', coding: 'Coding', image: 'Image Generation', business: 'Business',
  education: 'Education', marketing: 'Marketing', creative: 'Creative', productivity: 'Productivity',
  research: 'Research', analysis: 'Analysis', design: 'Design', other: 'Other',
};
...
export default async function ExplorePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-3xl font-bold text-gray-900 mb-8">Explore Prompts</h1>

      {/* AI Tools */}
      <section className="mb-12">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">By AI Tool</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {AI_TOOLS.map((tool) => (
            ...
            >
              ...
              <div>
                <p className="font-medium text-gray-900 text-sm">{AI_TOOL_LABELS[tool]}</p>
                <p className="text-xs text-gray-400">{aiMap[tool] || 0} prompts</p>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {CATEGORIES.map((cat) => (
            ...
            >
              ...
              <p className="text-xs text-gray-400">{catMap[cat] || 0} prompts</p>
            </Link>
          ))}
        </div>
      </section>
